...
#    m = re.match(".+_L8_(\d{3})_(\d{3}).hdr", file, re.IGNORECASE)
    ...
    m = re.match("SENTINEL2[A-D]_.+_L2A_T([A-Z0-9]+)_.+_MTD_ALL\.xml", file, re.IGNORECASE)
    if m:
#       return (Mission.LANDSAT, m.group(1) + m.group(2))
    ...
    m = re.match("S2[a-z0-9]_[a-z0-9]+_[a-z0-9]+_[a-z0-9]+_([a-z0-9]+)_.+\\.HDR", file, re.IGNORECASE)
    if m:
        return (Mission.SENTINEL, m.group(1))
